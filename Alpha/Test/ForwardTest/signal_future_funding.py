...
"""
파일 실행방법: mac: python3 -m Alpha.Test.ForwardTest.signal_future_funding
파일 실행방법: window: python -m Alpha.Test.ForwardTest.signal_future_funding
"""

INPUT_DIR = "./DB/forwardTest"
OUT_DIR = "./DB/Result"
...
def main():
    ...
    # =========================
    # fixed parameters (from backtest)
    VOL_ABS = 1
    ...
    # save result
    out_path = os.path.join(OUT_DIR, "signal_future_funding_forward.csv")
    bt.to_csv(out_path, index=False)

    print(f"[OK] saved: {out_path}")

    # simple forward summary
    summary = summarize_carry(bt, NOTIONAL)
    print(summary.to_string(index=False))

    print(
        funding_df.groupby("COIN")["FUNDING_RATE"].describe()[
            ["mean", "std", "min", "max"]
        ]
    )
    print(bt.groupby("COIN")[["PNL_FUNDING", "PNL_FEE"]].sum())
